//! Shared History-dock chrome for the SQL and Redis shells.
//!
...
//! [`HistoryPanelSpec`] and hand it to [`render`].
//!
...
//! two adapters thin and the collapse/search/nav policy where the state lives.
//!
...
use flint::prelude::*;
use gpui::{
    AnyElement, Context, Entity, FocusHandle, KeyDownEvent, SharedString, div, prelude::*, px,
};
...
pub(crate) type ActivateFn<V> = Rc<dyn Fn(&mut V, bool, &mut Context<V>)>;
...
pub(crate) type Action<V> = Rc<dyn Fn(&mut V, &mut Context<V>)>;
...
pub(crate) struct HistorySection<V> {
    /// Stable id for the header element (and the caller's collapse map key).
    pub key: &'static str,
    pub title: Option<SharedString>,
    pub collapsed: bool,
    pub toggle: Option<Action<V>>,
    pub rows: Vec<HistoryRow<V>>,
}
...
pub(crate) struct HistoryNav<V> {
    pub focus: FocusHandle,
    #[allow(clippy::type_complexity)]
    pub on_key: Rc<dyn Fn(&mut V, &KeyDownEvent, &mut Context<V>) -> bool>,
}

/// Everything the renderer needs to draw one History dock.
pub(crate) struct HistoryPanelSpec<V> {
    pub sections: Vec<HistorySection<V>>,
    ...
    pub show_clear: bool,
    pub on_clear: Action<V>,
    /// Hide the dock (the header ✕). A field rather than a hardcoded
    /// `toggle_history` call, because the SQL dock now asks its host to close it
    /// while the Redis dock still toggles `AppState` directly.
    pub on_close: Action<V>,
    ...
    pub nav: Option<HistoryNav<V>>,
    /// Flat index of the keyboard-highlighted row (matched against
    /// `HistoryRow::nav_index`).
    pub selected: Option<usize>,
    /// The hold-Alt focus-hint badge, built by the caller (only the app knows
    /// whether hints are showing and which key this surface drew).
    pub focus_badge: Option<AnyElement>,
}

/// Draw one History dock from a domain-free [`HistoryPanelSpec`].
pub(crate) fn render<V: 'static>(
    spec: HistoryPanelSpec<V>,
    cx: &mut Context<V>,
